...
<html>
	<body>
		<form action="index5.php" method="GET">
			<input type="radio" name="markSearch" value="<?php 3 or 4 or 5 ?>" 
			<?php
				if (isset($_GET['markSearch'])){
					echo 'checked';
				}
			?>>Любая
			<input type="radio" name="markSearch" value="5" 
			<?php
				if (isset($_GET['markSearch'])  && $_GET['markSearch'] == 5 ){
					echo 'checked';
				}
			?>>5		
			<input type="radio" name="markSearch" value="4" 
			<?php
				if (isset($_GET['markSearch'])  && $_GET['markSearch'] == 4 ){
					echo 'checked';
				}
			?>>4		
			<input type="radio" name="markSearch" value="3" 
			<?php
